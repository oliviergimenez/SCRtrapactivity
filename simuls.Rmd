...
biastrue <- biastrue %>%
  as_tibble() #%>%
#  pivot_longer(cols = density:scale, names_to = "par", values_to = "bias")
biasfalse <- biasfalse %>%
  as_tibble() #%>%
#  pivot_longer(cols = density:scale, names_to = "par", values_to = "bias")
...
df <- cbind(biastrue, grid) %>%
  ...
  labs(y = "Amount of inactive traps (in %)",
       x = "Inactive from occ 3 up to 8 (over 10 occ)",
       fill = "Relative bias (in %)",
       title = "Bias in density when trap activity is correct") + 
  scale_fill_gradient2(low = "red", 
                       mid = 0, 
                       high = "blue")

df2 <- cbind(biasfalse, grid) %>%
  as_tibble() %>%
  ggplot() + 
  aes(x = inactive.from.time, 
      y = percent.inactive.traps, 
  ...
  labs(y = "Amount of inactive traps (in %)",
       x = "Inactive from occ 3 up to 8 (over 10 occ)",
       fill = "Relative bias (in %)",
       title = "Bias in density when trap activity is incorrect") + 
  scale_fill_gradient2(low = "red", 
                       mid = 0, 
                       high = "blue")
